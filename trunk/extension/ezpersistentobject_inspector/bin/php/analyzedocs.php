...
foreach( $descriptions as $fetch => $description )
{
    ...
    list( $module, $fetch ) = explode( '/', $fetch );
    if ( !is_dir( ezFetchDocScanner::$storagedir."/$module" ) )
    {
        mkdir( ezFetchDocScanner::$storagedir."/$module" );
    }
    file_put_contents( ezFetchDocScanner::$storagedir."$module/$fetch.php", "<?php\n\$fetchdesc = " . var_export( $description, true ) . ";\n?>" );
}
